...
(function () {
	function insertImage(area, url) {
		area.focus();
		document.execCommand('insertHTML', false, '<img src="' + url.replace(/"/g, '&quot;') + '" alt="" style="max-width:320px;width:100%;height:auto;display:block;">');
	}
...
(function () {
	...
	function refreshNav() {
		...
		document.querySelectorAll("[data-crm-section]").forEach(function (a) {
			...
			if (current === section) {
				a.setAttribute("href", home);
				a.title = "Ir al inicio de esta secciÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ³n";
...
(function () {
	...
	if (mailForm) {
		mailForm.addEventListener("submit", function (event) {
			...
			if (mailStatus) mailStatus.textContent = "EnviandoÃÂ¢ÃÂÃÂ¦";
			fetch(base + "/correo", {
				method: "POST",
				body: body,
				credentials: "same-origin",
	...
	function contactLine(label, value) {
		return "<div><dt>" + label + "</dt><dd>" + (value || "ÃÂ¢ÃÂÃÂ") + "</dd></div>";
	}
	...
	function paintPerson(person) {
		...
		if (!box) return;
		if (!person) {
			box.innerHTML = "<p class=\"muted\">Elige quiÃÂÃÂ©n estÃÂÃÂ¡ a cargo.</p>";
		...
		box.innerHTML = ""
			...
			+ contactLine("Cargo", escapeHtml(person.title || "ÃÂ¢ÃÂÃÂ"))
			+ contactLine("TelÃÂÃÂ©fono", phone)
			+ contactLine("Correo", person.email ? escapeHtml(person.email) : "ÃÂ¢ÃÂÃÂ");
		if (mailBtn) {
	...
	function contactPicker(dealId, contacts) {
		if (!contacts.length) {
			return '<section class="kb-block"><h3>Contacto a cargo</h3><p class="muted">Este cliente no tiene contactos. AgrÃÂÃÂ©galos en sus datos.</p></section>';
		}
	...
	function renderDrawer(card, data) {
		...
		if (drawerKicker) drawerKicker.textContent = (client.name || "Cliente") + " ÃÂÃÂ· " + (deal.stage_label || "Prospecto");
		const notes = (data.notes || []).map(function (note) {
			const prefix = note.type === "recordatorio" ? "Recordatorio: " : "";
			return "<li><p>" + escapeHtml(prefix + note.message) + "</p><small>" + escapeHtml(note.who) + " ÃÂÃÂ· " + escapeHtml(note.when) + "</small></li>";
		}).join("");
		...
		drawerBody.innerHTML = ''
			...
			+ '<section class="kb-block"><h3>Cotizaciones de este proyecto</h3><ul class="kb-notes">' + (quotes || "<li><p>Sin cotizaciones todavÃÂÃÂ­a.</p></li>") + "</ul>"
			+ '<div class="kb-actions"><a class="is-primary" href="' + base + "/proyectos/" + id + '/cotizacion">Nueva cotizaciÃÂÃÂ³n</a>'
			+ '<form method="post" action="' + base + "/proyectos/" + id + '/eliminar" onsubmit="return confirm(\'ÃÂÃÂ¿Eliminar este proyecto? Se borran sus cotizaciones y notas. El cliente se mantiene.\');">'
		...
		if (openMail) {
			openMail.addEventListener("click", function () {
				...
				mailWho.textContent = (person.name || "Contacto") + (person.phone ? " ÃÂÃÂ· " + person.phone : "");
			...
			form.addEventListener("submit", function (event) {
				...
				fetch(base + "/tablero/proyecto/" + id + "/nota", {
					...
					.then(function (json) {
						...
						li.innerHTML = "<p>" + escapeHtml(json.note.message) + "</p><small>" + escapeHtml(json.note.who) + " ÃÂÃÂ· " + escapeHtml(json.note.when) + "</small>";
	...
	function openDrawer(card) {
		...
		drawer.hidden = false;
		drawerBody.innerHTML = '<p class="muted">CargandoÃÂ¢ÃÂÃÂ¦</p>';
